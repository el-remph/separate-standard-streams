...
doc: ssss.1
ssss.1: ssss
	printf '[NOTES]\nThis page auto-generated by help2man\n' | \
		help2man --no-info --include=- --output=$@ ./$^
release: ssss.tar
# Recommend compressing the tarball with tarlz(3) -- this requires tar(5)
# to be in pax format, eg.
#	make TAR='tar --format=pax -cf'
#	tarlz -9z -n`nproc` ssss.tar
ssss.tar: ssss ssss.1 README.md GPL
	$(TAR) $@ $^
...
clean:
	rm -fv ssss $(OBJS) config.h compat/unlocked-stdio.h ssss.1 ssss.tar
	make -C dryopt clean
