...
# unlocked_stdio(3)
for f in fwrite fileno fflush fgetws
do
	case $headers in
		*$f*)
			define_BSD=1 define_DARWIN=1
			# FIXME: these should be defined *after* the inclusion of stdio.h
			echo "#define $f ${f}_unlocked"
			chat "${f}_unlocked(3) found"
			;;
		*) chat "${f}_unlocked not found, using regular $f(3) instead" ;;
	esac
done
